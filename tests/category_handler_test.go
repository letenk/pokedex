...
func TestFindAllCategoryHandlerTest(t *testing.T) {
	// Test Cases
	testCases := []struct {
		name string
	...
	for i := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			request := httptest.NewRequest(http.MethodGet, "http://localhost:3000/api/v1/category", nil)
			...
			if tc.name == "success_with_role_admin" {
				...
				require.Equal(t, "list of category", responseBody["message"])
